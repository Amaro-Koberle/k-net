...
export default function Menu({
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false); // default just temporarily = true. Ultimately the default will be false.
  ...
  if (displayMenu === true) {
    return (
      <>
        <div className="fixed top-0 left-0 z-50 h-screen p-3 bg-opacity-75 shadow-2xl ring-1 ring-gray w-72 bg-gray-dark text-gray-lightest">
          ...
          {isLoggedIn ? (
            <UserMenu setEditingSettings={setEditingSettings} />
          ) : hasAccount ? (
            <AuthProvider>
              <LogIn
